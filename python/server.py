...
Obtiene las predicciones verificadas directamente del backend Node.js.
...
# Archivo local (solo como cache/backup, la fuente principal es el backend)
...
def fetch_predictions_from_backend() -> list:
    """Obtiene las predicciones verificadas directamente del backend"""
    try:
        url = f"{BACKEND_URL}/api/predictions?verified=true&limit=10000"
        req = urllib.request.Request(url, headers={'Accept': 'application/json'})
        with urllib.request.urlopen(req, timeout=10) as response:
            data = json.loads(response.read().decode('utf-8'))
            predictions = data.get('data', [])
            print(f"[Server] 📥 Obtenidas {len(predictions)} predicciones del backend")
            return predictions
    except urllib.error.URLError as e:
        print(f"[Server] ⚠️ Error conectando al backend: {e}")
        return []
    except Exception as e:
        print(f"[Server] ⚠️ Error obteniendo predicciones: {e}")
        return []
...
class TrainingHandler(BaseHTTPRequestHandler):
    ...
    def _get_predictions_count(self) -> int:
        """Cuenta predicciones verificadas del backend"""
        try:
            predictions = fetch_predictions_from_backend()
    ...
    def _train(self) -> dict:
        """Ejecuta el entrenamiento obteniendo predicciones del backend"""
        # Obtener predicciones del backend (fuente principal)
        predictions_data = fetch_predictions_from_backend()
        ...
        # Convertir a VerifiedPrediction (el backend usa camelCase)
        predictions = []
        for p in predictions_data:
            try:
                # El backend devuelve camelCase, así que mapeamos ambos formatos
                actual_change = p.get('actualChange', p.get('actual_change', 0)) or 0
                pred = VerifiedPrediction(
                    id=p.get('id', ''),
                    symbol=p.get('symbol', ''),
                    asset_type=p.get('assetType', p.get('asset_type', 'stock')),
                    timeframe_days=p.get('timeframeDays', p.get('timeframe_days', 1)),
                    predicted_direction=p.get('direction', p.get('predicted_direction', 'neutral')),
                    predicted_change=p.get('predictedChange', p.get('predicted_change', 0)) or 0,
                    predicted_price_min=p.get('predictedPriceMin', p.get('predicted_price_min', 0)) or 0,
                    predicted_price_max=p.get('predictedPriceMax', p.get('predicted_price_max', 0)) or 0,
                    confidence=p.get('confidence', 50),
                    price_at_prediction=p.get('currentPrice', p.get('current_price', 0)) or 0,
                    factor_scores=p.get('factorScores', p.get('factor_scores', {})) or {},
                    factor_weights=p.get('factorWeights', p.get('factor_weights', {})) or {},
                    actual_price=p.get('actualPrice', p.get('actual_price', 0)) or 0,
                    actual_change=actual_change,
                    actual_direction='up' if actual_change > 0 else ('down' if actual_change < 0 else 'neutral'),
                    direction_correct=p.get('directionCorrect', p.get('direction_correct', False)),
                    price_error=abs((p.get('predictedChange', p.get('predicted_change', 0)) or 0) - actual_change),
                    within_range=p.get('withinRange', p.get('within_range', False)),
                    prediction_date=p.get('createdAt', p.get('created_at', p.get('prediction_date', ''))),
                    verified_at=p.get('verifiedAt', p.get('verified_at', '')),
                )
